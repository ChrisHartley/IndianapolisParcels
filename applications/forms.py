...
class ApplicationForm(forms.ModelForm):
    Property = forms.ModelChoiceField(
        queryset=Property.objects.exclude(status__contains='Sale approved by MDC').exclude(is_active__exact=False).exclude(status__contains='Sold').exclude(status__contains='Sale approved by Board of Directors', renew_owned=True).order_by('streetAddress'),
    ...
    def validate_for_submission(self, *args, **kwargs):
        ...
        conflict_board_rc_name = cleaned_data.get('conflict_board_rc_name', None)
        tax_status_of_properties_owned = cleaned_data.get('tax_status_of_properties_owned', None)
        ...
        if conflict_board_rc is None:
            self.add_error("conflict_board_rc", ValidationError('This is a required question.'))

        if conflict_board_rc is True and conflict_board_rc_name == '':
            self.add_error("conflict_board_rc_name", ValidationError("You anwsered Yes above, please provide a name."))

        if tax_status_of_properties_owned is None:
            self.add_error('tax_status_of_properties_owned', ValidationError('This is a required question.'))

        if tax_status_of_properties_owned == Application.DELINQUENT_STATUS:
            self.add_error('tax_status_of_properties_owned',ValidationError("Delinquent tax payers are not eligible to purchase properties from Renew Indianapolis"))

        if active_citations is None:
            self.add_error('active_citations',ValidationError("This is a required question"))
        ...
        if Application.SIDELOT == application_type:
            ...
            if property_selected is not None and property_selected.structureType != "Vacant Lot":
                self.add_error('application_type', ValidationError('The property you have selected is not a vacant lot and hence is ineligible for our sidelot program.'))
        ...
        if Application.HOMESTEAD == application_type or Application.STANDARD == application_type:
            ...
            if UploadedFile.objects.filter(file_purpose__exact=UploadedFile.PURPOSE_SOW).filter(application__exact=app_id).count() == 0:
                self.add_error(None, ValidationError('You must upload a separate scope of work document with your application'))
            if UploadedFile.objects.filter(file_purpose__exact=UploadedFile.PURPOSE_POF).filter(application__exact=app_id).count() == 0:
                self.add_error(None, ValidationError('You must upload a separate proof of funds document with your application'))
            ...
            if Application.STANDARD == application_type:
                ...
                if not nsp_income_qualifier and property_selected.nsp and nsp_income_qualifier == "":
                    self.add_error('nsp_income_qualifier', ValidationError("Since this is a rental NSP property you must list who will be conducting tenant income qualification."))
                if property_selected is not None and property_selected.homestead_only:
                    self.add_error('application_type', ValidationError('The property you have selected is marked "homestead only" but you indicated a Standard application.'))
