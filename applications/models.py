...
class Application(models.Model):
	...
	organization = models.ForeignKey(Organization, null=True, blank=True, help_text="This is the name that will be shown on the deed, if ")
	...
	# neighborhood notification - boolean or file? boolean and then collection of files
...
class UploadedFile(models.Model):
	...
	FILE_PURPOSE_CHOICES = (
		...
		(PURPOSE_MOST_RECENT_FINANCIAL_STATEMENT, "Organization's Most Recent Financial Statement"),
		(PURPOSE_OTHER, 'Other')
	...
	created = models.DateTimeField(auto_now_add=True)
	supporting_document = models.FileField(upload_to="attachments/%Y/%m/%d", max_length=512)
	...
	def create_on_upload(sender, backend, request, **kwargs):
		...
		new_path = os.path.join(settings.MEDIA_ROOT, request.user.email, os.path.basename(backend.path))
		...
		UploadedFile.objects.create(file_purpose=request.GET['file_purpose'], supporting_document=new_path, user=request.user, application=app)
